...
class UserRepo {
    ...
    updateUser(id, user) {
        return typeorm_1.getRepository(users_1.users).createQueryBuilder().update().set(user).where("id = :id", { id: id }).execute();
    }
    updatePassword(id, data) {
        return typeorm_1.getRepository(users_1.users).createQueryBuilder().update().set({ passwordHash: data.passwordHash, salt: data.salt }).where("id = :id", { id: id }).execute();
    }
    deleteUser(id) {
        return typeorm_1.getRepository(users_1.users).createQueryBuilder().delete().where("id = :id", { id: id }).execute();
    }
