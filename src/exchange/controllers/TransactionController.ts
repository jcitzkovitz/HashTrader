...
export class TransactionController{
    ...
    async getAllTransactionsForUser(@Param("userId") userId:number){
        try{
            let response = await this.transactionRepo.getAllTransactions({where:{userId:userId}});
    ...
    async getAllTransactionsForUserWithCoin(@Param("userId") userId:number, @Param("coinId") coinId:number){
        try{
            let response = await this.transactionRepo.getAllTransactions({where:{userId:userId,coinId:coinId}});
            return new ResponseModel(true,'The transactions for this user have been selected',response);
        }catch(err){
    ...
    async getAllTransactions(){
        try{
            ...
            let response = await this.transactionRepo.getAllTransactions();
    ...
    async makeWithdrawal(@Body() body:WithdrawDepositModel){
        try{
            ...
            if(body.amount > user.withdrawalLimit) throw new UnauthorizedError("The user cannot withdraw an amount greater than their stated limit");

            //Change the users balance
            let balanceChange: BalanceChangeModel = {userId: user.id, coinId:body.coinId, change: "SUB",amount:body.amount};
            ...
            let txid = await this.addressRepo.withdraw(body.sendToAddress,fromAddressHash,body.amount);
            ...
            let response = await this.transactionRepo.saveTransation(transaction);
    ...
    async makeDeposit(@Body() body:WithdrawDepositModel){
        try{
            ...
            //Change the users balance
            let balanceChange: BalanceChangeModel = {userId: user.id, coinId:body.coinId, change: "ADD",amount:body.amount};
            let updatedBalanceResponse = await this.walletController.updateBalanceForCoin(balanceChange);
